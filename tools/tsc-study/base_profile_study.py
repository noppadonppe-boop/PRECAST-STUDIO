"""S2O base-wall profile refinement; old evidence immutable."""
...
OUT=ROOT/'output/tsc-step2o-r00';BP='knowledge/modular-tsc-step2o/basis.json';read=kernel.read
...
def build(g,t,m,ref):
    ...
    wall=kernel.split_knots(np.linspace(t,H-R,26),{0:4,1:4,2:4,23:2,24:2});arc=np.linspace(0,math.pi/2,49).tolist();roof=kernel.split_knots(np.linspace(R,B/2,23),{20:2,21:2});yy=np.linspace(0,L,17).tolist();cells=[]
...
def main():
    ...
    for p in [BP,b['upstream'],b['diagnostic_source'],'tools/tsc-study/base_profile_study.py']:deps[p]=sha(p)
    ...
    E=parent['material']['E_MPa']*1000;nu=parent['material']['nu'];pc=patch('20NodeBrick',E,nu);ident='BASE-T175-FR-FULL-NB';path='output/tsc-step2o-r00/'+ident+'.json';cache=OUT/'cache.json';fp=json.dumps(deps,sort_keys=True);saved=json.loads(cache.read_text()) if args.reuse and cache.exists() else None
    if not(saved and saved['fingerprint']==fp and sha(path)==saved['sha256']):
        ...
        types.FunctionType(kernel.run.__code__,scope)({'reference_mesh':{}},g,{'id':'NB'},parent,lb,cb);write('cache.json',{'fingerprint':fp,'sha256':sha(path)})
    raw=read(path);s=raw['summary'];affine=patch_check(raw,E,nu);tr=audit(raw,ab,E,nu);name='TRACTION-'+ident+'.json';write(name,tr);samples={'mesh':'NB','pattern':'FULL','samples':raw['samples']};del raw
    prev=read('output/tsc-step2m-r00/ARC-T175-FR-FULL-M1.json');comp=compare({'mesh':'M1','pattern':'FULL','samples':prev['samples']},samples,lb);del prev
    write('base_profile_results.json',{'id':b['id'],'revision':b['revision'],'status':b['status'],'basis':b,'inherited_basis':parent,'solver':up['solver'],'patch_check':pc,'affine_traction_patch':affine,'reference_runs':up['runs'],'runs':[s],'local_comparisons':[comp],'traction_runs':[up['traction_runs'][-1],tr],'dependency_hashes':deps,'raw_output_hashes':{path:sha(path),'output/tsc-step2o-r00/'+name:sha('output/tsc-step2o-r00/'+name)},'whole_model_local_stress_convergence':'NOT_ESTABLISHED','engineering_approval':False,'manufacturing_release':False})
    print('FINISHED NB cuts',sum(c['targets_met'] for c in tr['cuts']),'/6',flush=True)
