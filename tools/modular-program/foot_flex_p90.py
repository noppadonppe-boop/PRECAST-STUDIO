"""Elastic P89 candidate foot plate with unilateral bed/bolt springs; P83 one-way loads.
...
def run(key='A-LH-W01',owner='M01',foot_index=1,size=40,kc=1000.,length=50.,thickness=30.,washer_radius=18.5,base_revision='P89',prepare_only=False):
    base_path=ROOT/f"output/{'base-thread-candidate-p89' if base_revision=='P89' else 'abd-base-pattern-p85'}/{key}.json";wall_path=ROOT/f'output/wall-frame-p83/{key}-{owner}-h25.json'
    ...
    for b in bolts:
        root=node(*b['xy']);patch=[tag for tag in plate_ids if math.dist(coords[tag][:2],b['xy'])<=washer_radius+1e-5]
    ...
    for tag in plate_ids:
        anchor=1000000+tag;ops.node(anchor,*coords[tag]);ops.fix(anchor,1,1,1,1,1,1)
    ...
    for i,b in enumerate(root_bolts):
        anchor=2000000+i;ops.node(anchor,*coords[b['node']]);ops.fix(anchor,1,1,1,1,1,1)
    ...
    # Canonical FE-input representation, independent of node/element numbering.
    def xyzkey(p):return tuple(0. if abs(float(v))<.00005 else round(float(v),4) for v in p)
    def quadkey(c):
        q=[xyzkey(coords[i]) for i in c['ids']]
        return min(tuple(q[i:]+q[:i]) for i in range(4))
    canonical=dict(cells=sorted(quadkey(c) for c in cells),
        tributaries=sorted((xyzkey(coords[i]),round(float(a),6)) for i,a in tributary.items()),
        washerConstraints=sorted((xyzkey(coords[s]),xyzkey(coords[r])) for s,r in slaves.items()),
        roots=sorted((b['tag'],xyzkey(coords[b['node']])) for b in root_bolts),
        E=E,nu=nu,t=t,kb=kb,kc=kc)
    prepared=dict(key=key,foot=foot_tag,meshTargetMm=size,washerPatchRadiusMm=washer_radius,effectiveBoltLengthMm=length,
        geometryConstraintHash=hashlib.sha256(json.dumps(canonical,sort_keys=True).encode()).hexdigest(),
        loads=sorted((xyzkey(coords[i]),[float(x) for x in v]) for i,v in loads.items()),
        sourceInputs=[dict(path=p.relative_to(ROOT).as_posix(),sha256=sha(p)) for p in [base_path,wall_path]],
        plateNodeCount=len(plate_ids),shellCount=len(cells),washerPatchCounts=[len(b['washerNodes']) for b in root_bolts])
    ...
    return dict(revision='P90',baseGeometryRevision=base_revision,executionMethod='NATIVE_OPENSEES_SOLVE',preparedModel=prepared,key=key,owner=owner,foot=foot_tag,meshTargetMm=size,inputs=[dict(path=str(p.relative_to(ROOT)),sha256=sha(p)) for p in [base_path,wall_path]],solverSha256=sha(__file__),mesherSha256=sha(ROOT/'tools/modular-program/foot_mesh_p86.py'),solverVersion=ops.version(),originFabricationMm=origin.tolist(),basis=dict(E_MPa=E,nu=nu,thicknessMm=t,kContactNPerMm3=kc,kBoltNPerMm=kb,effectiveBoltLengthMm=length,stressAreaMm2=245,washerPatchRadiusMm=washer_radius),meshAudit=mesh_audit,nodes=nodes,cells=cells,bolts=root_bolts,boltForces=bolt_forces,reactions=reactions,loadMappings=mappings,appliedForceN=extF.tolist(),appliedMomentNmm=extM.tolist(),forceResidualN=(extF+reactF).tolist(),momentResidualNmm=(extM+reactM).tolist(),maxFreeVerticalResidualN=free_z,maxFreeBendingResidualNmm=free_m,maxUpwardDisplacementMm=max(n['displacementMm'][2] for n in nodes),minDisplacementMm=min(n['displacementMm'][2] for n in nodes),limitations=['One-way P83 support-load handoff, not coupled wall/foot redistribution.','In-plane DOFs restrained; shear carried separately, no membrane/shear-group response claim.',f'Rigid washer patch radius{washer_radius:g} mm is a kinematic idealization; real washer bending, bolt-head contact and thread capacity excluded.','Nearest material node mapping conserves vertical force and bending wrench; point/couple peaks are not capacity stresses.','Linear elastic plate with unilateral springs; no yielding, weld detail, manufacturing imperfections or approval.'],engineeringApproved=False,productionReleased=False,stageComplete=False)

if __name__=='__main__':
    import argparse
    ap=argparse.ArgumentParser()
    ap.add_argument('key');ap.add_argument('owner');ap.add_argument('index',type=int);ap.add_argument('mesh',type=float)
    ap.add_argument('--length',type=float,default=50.)
    ap.add_argument('--radius',type=float,default=18.5)
    ap.add_argument('--base',choices=['P85','P89'],default='P89')
    ap.add_argument('--prepare-only',action='store_true')
    a=ap.parse_args()
    r=run(a.key,a.owner,a.index,a.mesh,length=a.length,washer_radius=a.radius,base_revision=a.base,prepare_only=a.prepare_only)
    out=ROOT/'output/foot-flex-p90';out.mkdir(exist_ok=True,parents=True)
    name=f'{a.key}-{a.owner}-F{a.index}-h{a.mesh:g}-r{a.radius:g}-L{a.length:g}'
    if a.prepare_only:name+='-prepared'
    if a.base!='P89':name+='-'+a.base
    (out/(name+'.json')).write_text(json.dumps(r,indent=2),encoding='utf8')
    print(json.dumps(dict(file=name+'.json',prepared=a.prepare_only,boltForces=r.get('boltForces'),upliftMm=r.get('maxUpwardDisplacementMm'))),flush=True)
