...
export function build(key){const input=`output/abd-base-lock-p66/${key}.json`,raw=fs.readFileSync(path.join(root,input)),m=JSON.parse(raw),[family,side]=key.split('-'),s=family==='D'?200/2825:0,L=Math.hypot(1,s),stock=[];
 ...
 const actors=new Map(m.parts.map(p=>[p.tag,p.cells.map(cellFaces)])),extra=new Map(['M01','M03'].map(tag=>[tag,stock.filter(p=>p.owner===tag).flatMap(p=>p.solids)])),concrete=m.concrete.cells.map(cellFaces),clashes=[];
 ...
 return {revision:'P67',key,id:m.id,units:'mm',inputSnapshot:{path:input,sha256:crypto.createHash('sha256').update(raw).digest('hex')},source:m.source,stock,totalAddedKg:stock.reduce((a,g)=>a+g.massKg,0),audit:{staticHits:clashes,additionalIntegratedMoves:moves,baselineP66Passed:m.audit.staticHits.length===0&&m.audit.moves.every(v=>!v.hits.length)},scope:'WALL_FOOT_GUSSETS_ONLY',limitations:['Eight integral welded plates, not independent temporary supports.','Waler notch is nominal zero-clearance fit; welding access and fabrication tolerances remain unresolved.','No weld, plate buckling, restraint, bolt-group capacity or supported-handling approval.','Roof/end bracing and seam locks remain Stage5 tasks.'],engineeringApproved:false,productionReleased:false};
}
if(process.argv[1]===fileURLToPath(import.meta.url)){const out=path.join(root,'output/abd-wall-gusset-p67');fs.mkdirSync(out,{recursive:true});for(const k of (process.argv[2]?[process.argv[2]]:keys)){const m=build(k);fs.writeFileSync(path.join(out,k+'.json'),JSON.stringify(m,null,2));console.log(k,{static:m.audit.staticHits,moves:m.audit.additionalIntegratedMoves.filter(v=>v.hits.length),mass:m.totalAddedKg});}}
