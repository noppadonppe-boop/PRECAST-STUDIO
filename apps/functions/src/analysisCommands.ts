...
async function executeQueuedBenchmark(db: Firestore, command: QueueAnalysisRunCommand): Promise<AnalysisCommandResult> {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.update(runRef, { status: 'completed', phase: 'complete', result: output.result, verification: output.verification, outputHash, completedAt: now, phaseHistory: [...history, { phase: 'postProcess', status: 'completed', message: 'Normalized benchmark result created.' }, { phase: 'checks', status: 'completed', message: 'Equilibrium and benchmark evidence recorded; design remains NOT CHECKED.' }, { phase: 'artifacts', status: 'completed', message: 'Metadata checksum recorded; no large result arrays stored in Firestore.' }, { phase: 'complete', status: 'completed', message: 'Controlled benchmark run completed.' }] });
