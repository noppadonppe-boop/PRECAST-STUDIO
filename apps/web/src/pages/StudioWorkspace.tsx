...
export function StudioWorkspace() {
  ...
  if (mode !== 'emulator' && projectId && stageFor(gateId ?? 'g0', new URLSearchParams(location.search).get('view')).id === 'intake') return <BimIntake key={projectId} projectId={projectId} />;
  if (mode === 'shared' && projects.find((item) => item.id === projectId)?.exampleProfileId === 'test-module-v1') return <TestModuleWorkspace key={projectId} projectId={projectId!} />;
...
function PreviewWorkspace({ gateId }: { gateId: string }) {
  ...
  const criteriaRecord = useSharedDocument('criteria', `${projectId}-design-basis`, emptyDesignCriteria());
  const criteria = designCriteriaSchema.safeParse(criteriaRecord.data);
  const basisLabel = mode === 'shared' ? (criteriaRecord.revision ? `ร่าง R${criteriaRecord.revision}${criteriaRecord.dirty ? ' · ยังไม่บันทึก' : ''}` : 'ร่าง · ยังไม่บันทึก') : 'ร่างตัวอย่าง';
  ...
  return <div className="studio-engineering">
    {mode === 'shared' && stage.id !== 'criteria' && <><SharedMenuEditor key={stage.id} category={stage.id} id={`${projectId}-notes`} title={`ข้อมูลเมนู · ${stage.label}`} />
    ...
    <Surface className="studio-revisions" ariaLabel="Current revision context"><span>โครงการ <b>{project.code}</b></span><span>Source <b>{project.sourceRevision}</b></span><span>Design Basis <b>{basisLabel}</b></span><span>Model <b>{project.modelRevision}</b></span></Surface>
    ...
    {stage.id !== 'criteria' && <button className="studio-inspector-toggle button button--secondary" type="button" aria-expanded={inspectorOpen} aria-controls="studio-inspector" onClick={() => setInspectorOpen(!inspectorOpen)}>{inspectorOpen ? 'ซ่อน' : 'แสดง'}คุณสมบัติ</button>}</div>
    <div className={`studio-engineering-grid ${inspectorOpen && stage.id !== 'criteria' ? '' : 'studio-inspector-hidden'}`}>
      <div className="studio-working-surface">
        ...
        {['analysis', 'design', 'report'].includes(stage.id) && criteria.success && <PrestressCalculator value={criteria.data} />}
        {stage.id === 'criteria' && (criteria.success ? <DesignCriteriaEditor value={criteria.data} onChange={criteriaRecord.setData} disabled={!criteriaRecord.loaded || criteriaRecord.saving} actions={<><div className="dialog-actions"><button type="button" className="button button--primary" disabled={mode !== 'shared' || !criteriaRecord.loaded || criteriaRecord.saving || !criteriaRecord.dirty} onClick={() => void criteriaRecord.save()}>บันทึกร่าง Design Basis</button>{mode === 'shared' && <button type="button" className="button button--secondary" disabled={!criteriaRecord.loaded || criteriaRecord.saving} onClick={criteriaRecord.reload}>โหลด Design Basis ล่าสุด</button>}<button type="button" className="button button--secondary" disabled>ส่งตรวจ Design Basis</button></div><p role={criteriaRecord.error ? 'alert' : 'status'}>{criteriaRecord.error || (mode !== 'shared' ? 'โหมดตัวอย่าง — ปรับค่าเพื่อทดลองได้ ข้อมูลยังไม่บันทึกถาวร' : !criteriaRecord.loaded ? 'กำลังอ่าน Design Basis…' : criteriaRecord.saving ? 'กำลังบันทึก…' : criteriaRecord.dirty ? 'มีการแก้ไขที่ยังไม่บันทึก · ต้องตรวจผลที่อ้างอิงเกณฑ์นี้ใหม่' : `บันทึกแบบร่างร่วม · Revision ${criteriaRecord.revision}`)}</p><p>การส่งตรวจและอนุมัติใช้กระบวนการควบคุมสิทธิ์ของโครงการ โหมดนี้บันทึกได้เฉพาะร่าง</p></>} /> : <Surface><p role="alert">รูปแบบข้อมูล Design Basis ไม่ถูกต้อง กรุณาตรวจข้อมูลที่บันทึกไว้ก่อนแก้ไข</p></Surface>)}
        ...
        {stage.id === 'report' && <Surface className="studio-report-layout"><nav aria-label="สารบัญรายการคำนวณ">{sections.map((title, index) => <button type="button" key={title} aria-current={reportSection === index ? 'page' : undefined} onClick={() => setReportSection(index)}>{index + 1}. {title}</button>)}</nav><article className="studio-paper"><div className="studio-paper-head">PRECAST STUDIO <span>DRAFT · ตัวอย่าง</span></div><p className="eyebrow">CALCULATION REPORT</p><h2>{reportSection + 1}. {sections[reportSection]}</h2><p>{project.name}</p><dl className="studio-facts"><div><dt>Design Basis</dt><dd>{basisLabel}</dd></div><div><dt>Model</dt><dd>{project.modelRevision}</dd></div><div><dt>ผลตรวจ</dt><dd>NOT CHECKED</dd></div></dl><p>พื้นที่แสดงรายการคำนวณจากผลที่ตรวจสอบแล้ว รายงานตัวอย่างนี้ยังไม่มีผล FEM หรือข้อสรุปออกแบบสำหรับออกเอกสารจริง</p><footer>ร่างประกอบ UX/UI · ห้ามใช้ก่อสร้าง</footer></article></Surface>}
      ...
      <aside className="studio-inspector" id="studio-inspector" aria-label="คุณสมบัติและการตรวจสอบ" hidden={!inspectorOpen || stage.id === 'criteria'}>
