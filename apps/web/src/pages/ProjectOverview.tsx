    ...
    <>
      ...
      <div className="project-layout">
        ...
        <div className="project-main-column">
          ...
          <Surface className="mock-analysis"><div><span className="mock-analysis__icon"><Icon name="cube" /></span><div><p className="eyebrow">M4 ENGINEERING BOUNDARY</p><h2>Controlled benchmark evidence</h2></div></div><StatusBadge tone="warning">{deterministicMockAnalysis.status}</StatusBadge><p>Versioned benchmark evidence does not constitute an engineering design check or G3 approval. {deterministicMockAnalysis.disclaimer}</p><dl><div><dt>Fixture engine</dt><dd>{deterministicMockAnalysis.engine}</dd></div><div><dt>Schema</dt><dd>{deterministicMockAnalysis.schemaVersion}</dd></div><div><dt>Input hash</dt><dd>{deterministicMockAnalysis.inputHash.slice(0, 24)}…</dd></div><div><dt>Output hash</dt><dd>{deterministicMockAnalysis.outputHash.slice(0, 24)}…</dd></div></dl></Surface>
        </div>
      </div>
    </>
  );
